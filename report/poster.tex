...
\node[panel,text width=8.0cm,anchor=north west,fill=abblight] at ([yshift=-7pt]p1.south west) (p2){%
  \hd{解决方案与定位}\\[2pt]
  本地量化大模型 + 官方手册 RAG，在工业 PC（CPU、无 GPU）上把\textbf{一句中文}变成
  符合 ABB 规范、可导入 RobotStudio 的 \textbf{.mod} 程序；\textbf{数据全程不出厂}。};
\node[panel,text width=8.0cm,anchor=north west] at ([yshift=-7pt]p2.south west) (p3){%
  \hd{七大创新点}\\[2pt]
...
% 左栏底部：真实落地实景（现场实拍）
\node[panel,text width=8.0cm,anchor=north west,fill=white] at ([yshift=-7pt]p3.south west) (pimg){%
  \hd{真实落地实景（现场实拍）}\\[3pt]
  \centering
  \includegraphics[height=2.6cm]{field-robot.jpg}\hspace{3pt}\includegraphics[height=2.6cm]{field-qc.jpg}\\[2pt]
  {\scriptsize\color{abbgray} 机器人执行本工具生成的程序喷涂 · 成品膜厚检测 45.9–61.3\,$\mu$m}};
...
\node[panel,fit=(aTitle)(aCli)(aOrc)(aR)(aL)(aP)(aKb),inner sep=8pt] (march){};
\end{scope}
\node[panel,text width=8.0cm,anchor=north west] at ([yshift=-8pt]march.south west) (m2){%
...
\node[panel,text width=8.0cm,anchor=north west,fill=abblight] at ([yshift=-7pt]m2.south west) (m3){%
  \hd{Pack\&Go 一键交付}\\[2pt]
...
\node[panel,text width=8.05cm,anchor=north west] at (19.05,\cyA) (r1){%
  ...
  \end{tabular}};
\node[panel,text width=8.05cm,anchor=north west] at ([yshift=-7pt]r1.south west) (r2){%
  \hd{生成—校验闭环（实跑实录）}\\[2pt]
  对缺陷代码精确拦截，错误码可追溯：\\
  {\ttfamily\scriptsize\color{abbdark}[ERROR][PNT001] 缺 brushdata 形参\\
  $[$ERROR$][$IO001$]$ 信号不在 EIO.cfg 白名单}\\
  把现场才暴露的 40212 错误\textbf{左移到生成时}。};
\node[panel,text width=8.05cm,anchor=north west,fill=boxblue] at ([yshift=-7pt]r2.south west) (r3){%
  \hd{开源 \& 在线演示}\\[3pt]
  \begin{minipage}[t]{5.35cm}\footnotesize 源码\,github.com/Hollis36/abb-offline-coder\\
  演示\,hollis36.github.io/abb-offline-coder\\[1pt]
  \scriptsize Python 87\%\, ·\, MIT\, ·\, rag/ollama/qwen/chromadb\end{minipage}\hfill
  \begin{minipage}[t]{2.15cm}\centering
  \qrcode[height=1.75cm]{https://hollis36.github.io/abb-offline-coder/}\\[1pt]
  {\tiny\color{abbgray}扫码看在线演示}\end{minipage}};
