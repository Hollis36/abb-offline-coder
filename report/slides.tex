% =====================================================================
%  答辩幻灯 —— abb-offline-coder   (Beamer 16:9)
...
% 中英文统一到 Noto 家族（英文/数字与中文同源，排版更整齐）
\setsansfont{Noto Sans CJK SC}[BoldFont={Noto Sans CJK SC:style=Bold},AutoFakeSlant=0.18]
\setmonofont{Noto Sans Mono CJK SC}[BoldFont={Noto Sans Mono CJK SC:style=Bold}]
% 防止单个中文字单独换行到行尾/行首（自动从上一行拉回一个字）
...
\end{column}
\end{columns}
\takeaway{一条主线：\textbf{痛点 $\to$ 方案 $\to$ 技术 $\to$ 验证 $\to$ 价值}。}
\end{frame}
...
\textbf{云端 AI 编程工具}（Copilot / ChatGPT 等）\textbf{在喷涂现场无法落地}，现场数据也\textbf{不能外传}。};
\end{tikzpicture}
\end{column}
\end{columns}
\takeaway{云端用不了、数据不能外传 $\Rightarrow$ 需要\textbf{完全离线、又懂喷涂}的中文 AI 助手。}
\end{frame}
...
用户用\textbf{中文}描述需求，产出符合 ABB 规范、可导入 RobotStudio 的 \texttt{.mod}。
\end{column}
\begin{column}{0.54\textwidth}
\textbf{\color{abbred}怎么做到离线}
\begin{itemize}
  \item 本地 LLM：Qwen2.5-Coder，\mbox{纯 CPU 可跑}
  \item 官方手册 RAG：向量 + BM25 检索
  \item 数据\textbf{全程不出厂}
\end{itemize}
\end{column}
\end{columns}
\takeaway{中文一句话 $\to$ 可上机 \texttt{.mod}；\textbf{本地模型 + 手册 RAG，数据不出厂}。}
\end{frame}
...
\end{column}
\end{columns}
\takeaway{纯函数、数据不可变、副作用集中于 Ollama/ChromaDB 边界 $\to$ \textbf{易测、可演进、能离线}。}
\end{frame}
...
\end{column}
\end{columns}
\takeaway{RAG 让小模型\textbf{不乱编}；校验闭环把现场错误\textbf{提前到生成时}拦下。}
\end{frame}
...
\begin{itemize}\small
  ...
  \item \textbf{IRC5P}：\texttt{PaintL/PaintC + brushdata}
\end{itemize}
\texttt{controller} 贯穿全流程，是一等参数。
\end{column}
\begin{column}{0.46\textwidth}
\textbf{\color{abbred}Pack\&Go 一键交付}\\[3pt]
一句需求 → 可直接加载的完整目录：
\begin{itemize}\small
  \item \texttt{.mod、BASE.sys、.pgf、README}
  \item 自动处理 ISO-8859-1 / CRLF 编码
\end{itemize}
\end{column}
\end{columns}
\takeaway{同一句话适配两种控制器，\textbf{一键产出可直接加载的目录}，贴合现场交付。}
\end{frame}
...
\end{column}
\end{columns}
\takeaway{\textbf{156 测试通过、核心覆盖 92–100\%}，且秒级可复现——工程质量可验证。}
\end{frame}
...
\end{column}
\end{columns}
\takeaway{一句话\textbf{自动补全声明与安全模板}，过校验即可交付。}
\end{frame}
...
\end{columns}
\takeaway{本工具生成的程序\textbf{已在真实无网喷涂车间现场运行并通过质检}。}
\end{frame}
