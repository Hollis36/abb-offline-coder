...
> ✅ **状态：已集成**。已从推送的现场照片中选用 4 张并裁剪为
> `field-cell.jpg`（玻璃喷房）、`field-robot.jpg`（机器人执行程序喷涂）、
> `field-part.jpg`（喷涂成品）、`field-qc.jpg`（膜厚检测），
> 已加入幻灯（新增「真实应用现场」一页）与研究报告（§8.3 真实落地），并重生成合订本/全套包。
> 其余 hash 命名的原图为推送的原始素材，可按需保留或精简。
